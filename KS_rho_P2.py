...
if __name__ == '__main__':
    ...
    rhogas_2D_all = np.zeros((len(M200c_array), len(c_dm_vals), nrp))
    ...
    for jM in tqdm(range(len(M200c_array))):
        ...
        for jc in range(len(c_dm_vals)):
            ...
            rho_gas = rhogas_r(rp_array, gammav, c_dm_vals[jc], Mvir=M200c, fb_cosmo=cosmo.Ob0 / cosmo.Om0)
            ...
            gamma_all[jc] = gammav
            print(time.time() - ti)
    np.savez('test1', P2d=Pgas_2D_all, P3d=Pgas_3D_all, gamma=gamma_all, rho2d=rhogas_2D_all)
